...
def data_sampleing(data, n):
    ...
    times0, activations0, volumes0, pressures0, aortic_pressures0, outflows0 = filling_phase_sampling(data, segment_i[0], segment_f[0])
    times1, activations1, volumes1, pressures1, aortic_pressures1, outflows1 = isovolumetric_phase_sampling(data, segment_i[1], segment_f[1], n1)
    times2, activations2, volumes2, pressures2, aortic_pressures2, outflows2 = ejection_phase_sampling(data, segment_i[2], segment_f[2], n2)
    times3, activations3, volumes3, pressures3, aortic_pressures3, outflows3 = isovolumetric_phase_sampling(data, segment_i[3], segment_f[3], n3)

    sampled_data = {
        "time": np.concatenate(([data['time'][0]], times0, times1, times2, times3)).tolist(),
        "activation": np.concatenate(([data['activation'][0]], activations0, activations1, activations2, activations3)).tolist(),
        "volume": np.concatenate(([data['volume'][0]], volumes0, volumes1, volumes2, volumes3)).tolist(),
        "lv_pressure": np.concatenate(([data['lv_pressure'][0]], pressures0, pressures1, pressures2, pressures3)).tolist(),
        "aortic_pressure": np.concatenate(([data['aortic_pressure'][0]], aortic_pressures0, aortic_pressures1, aortic_pressures2, aortic_pressures3)).tolist(),
        "outflow": np.concatenate(([data['outflow'][0]], outflows0, outflows1, outflows2, outflows3)).tolist() 
    }
...
def data_segmenting(data):
    ...
    segment_i = [1, systole_indice, non_zero_outflow_indices[0], non_zero_outflow_indices[-1]+1]
    segment_f = [systole_indice-1, non_zero_outflow_indices[0]-1, non_zero_outflow_indices[-1], len(outflow)]
...
def filling_phase_sampling(data, start, end):
    return([data['time'][end]],
           [data['activation'][end]],
           [data['volume'][end]],
           [data['lv_pressure'][end]],
           [data['aortic_pressure'][end]],
           [data['outflow'][end]])
...
def isovolumetric_phase_sampling(data, start, end, num_samples):
    ...
    sampled_pressure = np.linspace(np.min(segment_pressure)*1.01, np.max(segment_pressure)*.99, num_samples)
    
    time_interp = interp1d(segment_pressure, segment_time, kind='linear', fill_value="extrapolate")
    activation_interp = interp1d(segment_pressure, segment_activation, kind='linear', fill_value="extrapolate")
    aortic_pressure_interp = interp1d(segment_pressure, segment_aortic_pressure, kind='linear', fill_value="extrapolate")
    outflow_interp = interp1d(segment_pressure, segment_outflow, kind='linear', fill_value="extrapolate")
    
    # Create a structured array to reorder based on original time sequence
    sampled_data = np.array(list(zip(time_interp(sampled_pressure), activation_interp(sampled_pressure), np.full(num_samples, mean_volume), sampled_pressure, aortic_pressure_interp(sampled_pressure), outflow_interp(sampled_pressure))),
                            dtype=[('time', float), ('activation', float), ('volume', float), ('lv_pressure', float), ('aortic_pressure', float), ('outflow', float)])

    # Sort by 'time' to maintain the semblance of the original data order
    sampled_data_sorted = np.sort(sampled_data, order='time')

    return (sampled_data_sorted['time'],
...
def ejection_phase_sampling(data, start, end, num_samples):
    ...
    aortic_pressure_interp = CubicSpline(segment_volumes_sorted, segment_aortic_pressure_sorted)
    outflow_interp = CubicSpline(segment_volumes_sorted, segment_outflow)
    
    # Create a structured array to reorder based on original time sequence
    sampled_data = np.array(list(zip(time_interp(sampled_volumes), activation_interp(sampled_volumes), sampled_volumes, pressure_interp(sampled_volumes), aortic_pressure_interp(sampled_volumes), outflow_interp(sampled_volumes))),
                            dtype=[('time', float), ('activation', float), ('volume', float), ('lv_pressure', float), ('aortic_pressure', float), ('outflow', float)])
...
def data_plotting(data, style = 'ro'):
    ...
    ax2.plot(data['volume'], pressures_mmHg, style, alpha=0)  # invisible plot just for axis
    ...
    axs[1, 1].plot(data['time'], data['aortic_pressure'], style, label="Aortic Pressure")
    axs[1, 1].legend()
    axs[1, 1].set_ylabel("Pressure (kPa)")
    axs[1, 1].set_xlabel("Time (ms)")
    ax4 = axs[1, 1].twinx()
    ax4.plot(data['time'], pressures_mmHg, style, alpha=0)  # invisible plot just for axis
